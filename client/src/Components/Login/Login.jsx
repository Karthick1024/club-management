    ...
    const handleSubmit = async (e) => {
        ...
        try {
          const response = await axios.post(
            `${url}/login`,
            { ...formData },  // Send the form data (name and password)
            { headers: { 'Content-Type': 'application/json' } }
          );
      
          // Store the token and role in localStorage for later use
          ...
          if (user.role === 'hod') {
            window.location.href = '/dashboard'; // Assuming the dashboard route
          } else if (user.role === 'staff') {
            window.location.href = '/dashboard'; // Same route for staff
          } else {
